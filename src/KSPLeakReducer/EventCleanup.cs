    ...
    internal static class EventCleanup
    {
        ...
        internal static void RemoveGameEvent(object eventSource, object owner, Type handlerDeclaringType, string methodName)
        {
            ...
            if (handlerMethod == null)
            {
                ...
                Debug.LogWarning("[KSPLeakReducer] Missing handler " + typeName + "." + methodName);
                return;
            }
            ...
            {
                Debug.LogWarning("[KSPLeakReducer] Missing Remove(delegate) on " + eventSource.GetType().FullName);
                return;
            }
            ...
            if (handler == null)
            {
                Debug.LogWarning("[KSPLeakReducer] Could not bind " + owner.GetType().FullName + "." + methodName + " as " + delegateType.FullName);
                return;
            }
        ...
        internal static void RemoveInstanceEventField(object source, string fieldName, object owner, string methodName)
        {
            ...
            if (field == null)
            {
                Debug.LogWarning("[KSPLeakReducer] Missing field " + source.GetType().FullName + "." + fieldName);
                return;
            }
        ...
        internal static void RemoveStaticDelegateField(Type type, string fieldName, object owner, string methodName)
        {
            ...
            if (field == null)
            {
                Debug.LogWarning("[KSPLeakReducer] Missing static delegate field " + type.FullName + "." + fieldName);
                return;
            }
            ...
            if (handlerMethod == null)
            {
                Debug.LogWarning("[KSPLeakReducer] Missing handler " + owner.GetType().FullName + "." + methodName);
                return;
            }
            ...
            if (handler == null)
            {
                Debug.LogWarning("[KSPLeakReducer] Could not bind " + owner.GetType().FullName + "." + methodName + " as " + field.FieldType.FullName);
                return;
            }
        ...
        internal static void RemoveDelegatesOwnedBy(object eventOwner, string delegatePropertyName, object callbackOwner)
        {
            ...
            if (property != null)
                ...
                Debug.LogWarning("[KSPLeakReducer] Missing delegate member " + type.FullName + "." + delegatePropertyName);
        ...
        internal static int RemoveDestroyedOwners(object eventSource, Type ownerType)
        {
            ...
            if (eventsField == null)
            {
                Debug.LogWarning("[KSPLeakReducer] Missing events list on " + eventSource.GetType().FullName);
                return 0;
            }
            ...
            for (int i = events.Count - 1; i >= 0; i--)
            {
                ...
                {
                    Debug.LogWarning("[KSPLeakReducer] Missing event originator field on " + eventEntry.GetType().FullName);
                    return removed;
                }
        ...
        internal static int RemoveDestroyedStockGameEventOwners()
        {
            ...
            {
                Debug.LogWarning("[KSPLeakReducer] Missing BaseGameEvent.eventsByName");
                return 0;
            }
            ...
            {
                KSPLeakReducerSettings.LogDebug("Deferred broad stock sweep because the GameEvents registry changed");
                return 0;
            }
        ...
        internal static int RemoveDestroyedDelegateMemberOwners(object source, string delegateMemberName)
        {
            ...
            Delegate cleaned = current;
            int removed = 0;
            Dictionary<string, int> removedByType = KSPLeakReducerSettings.VerboseDebugLogging
        ...
        {
            if (!KSPLeakReducerSettings.VerboseDebugLogging || removed <= 0) return;

            Debug.Log("[KSPLeakReducer:Debug] Removed " + removed + " callback(s) from " + eventName + " owned by " + ownerTypeName + " via " + strategy);
        }
